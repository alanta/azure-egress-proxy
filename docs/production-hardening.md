...
| **Front Door Standard** → sample app with external ingress locked to `AzureFrontDoor.Backend` + `X-Azure-FDID` check (filtered-public origin) | **Front Door Premium + Private Link origin** to an internal-only Container Apps environment — no public origin at all |
| Sample app reachable for demo purposes | Internal-only ingress; reach it over private connectivity |
