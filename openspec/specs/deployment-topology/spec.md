...
How the deployment is divided into zones, and what each zone is and is not allowed to reach.

The shape is a security control, not a convention. Three zones — hub (the proxy and the audit
trail), spoke (the workloads), management (the control plane and the console) — with the
management zone peering to nothing, so reaching the proxy from it is not a matter of policy but
of routing. The requirements below fix the properties that make the control hold: that
enforcement is the subnet's deny rule rather than a workload's cooperation, that egress leaves
through an enumerable set of addresses, that artifacts exist before the compute that fetches
them, and that storage is written with Entra credentials only.
## Requirements
### Requirement: Enforcement is the network floor, not the workload's cooperation
...
### Requirement: The proxy subnet is deliberately unrestricted outbound
...
### Requirement: Traffic leaves through a known, enumerable set of addresses
...
### Requirement: Workloads reach the proxy by name over a private path
