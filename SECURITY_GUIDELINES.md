...
CI has credentials — OIDC federation to Azure, `packages: write` to GHCR, `contents: write`
for releases. A workflow compromise is a supply-chain compromise, so workflows get the same
...
- Use OIDC (`id-token: write` plus `azure/login`) for cloud auth. Do not add long-lived
  cloud credentials as repository secrets.
...
```bash
dotnet restore AzureEgressProxy.slnx --force-evaluate   # after any package change
```
...
Go has the same guarantee through `go.sum`, which records a hash for every module. Commit
`go.mod` and `go.sum` together; never hand-edit either. Use `go get` and `go mod tidy`.
...
2. Bump **indirect** dependencies directly when the advisory names them — you do not have to
   wait for the intermediate package to update. In Go,
   `go get golang.org/x/net@vX.Y.Z && go mod tidy` does this.
...
- [ ] `go.mod` and `go.sum` committed together, produced by tooling
